...
#' @description Converts all numerical variables into factor or character, depending on 'stringsAsFactors' parameter, based on equal frequency criteria.
#' The thresholds for each segment in each variable are generated based on the output of \code{\link{discretize_get_bins}} function, which returns a data frame
...
#' More info about converting numerical into categorical variables can be found at: \url{https://livebook.datascienceheroes.com/data-preparation.html#data_types}
...
#' @param digits number of significant digits (same parameter as defined by R 'digits')
...
#' @description It takes a data frame and returns another data frame indicating the threshold for each bin (or segment) in order to discretize the variable.
...
#'  If empty it will run for all numerical variables that match the following condition, the number of unique values must be higher than the ones defined at 'n_bins' parameter.
#'  NAs values are automatically handled by converting
#'  them into another category (more info about it at \url{https://livebook.datascienceheroes.com/data-preparation.html#treating-missing-values-in-numerical-variables}).
