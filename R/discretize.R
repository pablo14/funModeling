...
discretize_df <- function(data, data_bins, stringsAsFactors=T)
{
	...
	if(stringsAsFactors)
	{
		data_2b=data %>% mutate(across(all_of(vars_num), conv_factor))
		data_3=data_2b %>% mutate(across(all_of(vars_num), ~factor(replace(., is.na(.), "NA."))))
	} else {
		data_3=data %>% mutate(across(all_of(vars_num), ~ifelse(is.na(.), "NA.", .)))
	}
...
convert_df_to_categoric <- function(data, n_bins)
{
	...
	data_cat_2=data_cat %>% mutate(across(everything(), as.character))
